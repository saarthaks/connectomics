...
def cluster_dist(similarity_dict_part, shuffled_branches, pt_root_id_to_char):
    ...
    for cell_id, v in tqdm(similarity_dict_part.items()):
        ...
        for branch_id_1, other_branches in branch_comparisons.items():
            dists = BranchSeq.cluster_dist_list(shuffled_branches[cell_id][branch_id_1], other_branches, pt_root_id_to_char)
...
def main(branch_path, similarity_path, cells_path, trial_path, shuffle_type, num_trials, 
            score_dict_path=None, verbose=False):
    ...
    for t in range(num_trials):
        ...
        all_dists = cluster_dist(similarity_dict, shuffled_branches, pt_root_id_to_char)
        if verbose:
...
if __name__ == '__main__':
    ...
    main(args.branches_path, args.similarity_path, args.cells_path, args.trial_path, 
         ...
         score_dict_path=args.score_dict_path, verbose=args.verbose)
